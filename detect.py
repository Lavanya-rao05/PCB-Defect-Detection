...
def run(
    ...
    # Save the processed images temporarily (or return paths)
    grayscale_path = 'static/processed/grayscale.jpg'
    edges_path = 'static/processed/edges.jpg'
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if det is None or len(det) == 0:
                LOGGER.info("No PCB detected in the image.")
                
                # Save the message to a result file or pass back via Flask if needed
                result_img = im0s.copy()  # original image
                cv2.putText(result_img, 'No PCB defect Detected', (50, 50),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)
                
                # Save the annotated image in 'static/result'
                fallback_path = f'static/result/{Path(source).name}'
...
def Start(File):
    ...
    return {"status": True, "message": "Detection completed"}
